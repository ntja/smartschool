...
(function($) {
    $(document).ready(function() {
        ...
        if (localStorage.admin_token) {
            ...
            $("#ajouter").attr('href', root_admin + '/settings/current-status/add');
    ...
    function getStatus(param){
        ...
        $.fn.dataTable.ext.errMode = 'throw'; //throw error on console
           ...
           var table = $('#listJobs').DataTable( {
                ...
                "searching": true,
                ajax:{
                    url: "../js/settings/currentstatus/server-side-processing.php", // Change this URL to where your json data comes from
                ...
                "columnDefs": [                            
                    {
                        ...
                        "render": function ( data, type, row ) { 
                            ...
                            html += "<li><a href='"+root_admin+"/settings/current-status/"+row[2]+"' class='userProfil'>Edit</a></li>";
            ...
            function formattage(i, id_status) {
                try {            
                    ...
                    $('#listJobs > tbody > tr:nth-child(' + i + ') > td:nth-child(3) > #action_edit_delete > ul > li:nth-child(5) > a').attr('data-reject', id_status);
                    $('#listJobs > tbody > tr:nth-child(' + i + ') > td:nth-child(3) > #action_edit_delete > ul > li:nth-child(3) > a').attr('data-reject', id_status);            
    ...
    function disable_status(id, line) {
        try {
            var uri;

            uri = config.api_url + '/current-status/' + id + '/disapprove';
            console.log(uri);
            $.ajax({
                url: uri,
                method: "POST",
    ...
    function activate_status(id, line) {
        try {
            var uri;

            uri = config.api_url + '/current-status/' + id + '/approve';
            console.log(uri);
            $.ajax({
                url: uri,
                method: "POST",
