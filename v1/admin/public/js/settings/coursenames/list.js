...
(function($) {
    $(document).ready(function() {
        ...
        if (localStorage.admin_token) {
            ...
            $("#ajouter").attr('href', root_admin + '/settings/course-names/add');
            ...
            $('#deleteJob').click(function() {
                ...
                console.log(line);
                disable_coursename(id_skill, line);
            });                                
            ...
            $('#activateCt').click(function() {
                ...
                activate_coursename(id_ct, line);
            });
    ...
    function getCoursename(param){
        ...
        $.fn.dataTable.ext.errMode = 'throw'; //throw error on console
           ...
           var table = $('#listJobs').DataTable( {
                ...
                "searching": true,
                ajax:{
                    url: "../js/settings/coursenames/server-side-processing.php", // Change this URL to where your json data comes from
                ...
                "columnDefs": [                            
                    {
                        ...
                        "render": function ( data, type, row ) { 
                            ...
                            html += "<li><a href='"+root_admin+"/settings/course-names/"+row[2]+"' class='userProfil'>Edit</a></li>";
    ...
    function formattage(i, id_course_name) {
        try {  
            ...
            $('#listJobs > tbody > tr:nth-child(' + i + ') > td:nth-child(3) > #action_edit_delete > ul > li:nth-child(5) > a').attr('data-reject', id_course_name);
            $('#listJobs > tbody > tr:nth-child(' + i + ') > td:nth-child(3) > #action_edit_delete > ul > li:nth-child(3) > a').attr('data-reject', id_course_name);            
    ...
    function activate_coursename(id, line) {
        try {
            var uri;

            uri = config.api_url + '/course-names/' + id + '/approve';
            console.log(uri);
            $.ajax({
                url: uri,
                method: "POST",
...
//							}
    ...
    function disable_coursename(id, line) {
        try {
            var uri;

            uri = config.api_url + '/course-names/' + id + '/disapprove';
            console.log(uri);
            $.ajax({
                url: uri,
                method: "POST",
...
//                            $('#listJobs > tbody > tr:nth-child(' + line + ')').attr('hidden', 'hidden');
    ...
    /*
    function get_course_names(direction) {
        try {
            var filters, status;
    ...
    function formattage(count) {
        try {
            for (i = 1; i <= count; i++) {
                ...
                $('#listJobs > tbody > tr:nth-child(' + i + ') > td:nth-child(2) > #action_edit_delete > ul > li:nth-child(1) > a').attr('href', root_admin + '/settings/course-names/' + id_job);
