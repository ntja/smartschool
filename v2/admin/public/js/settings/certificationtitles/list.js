...
(function($) {
    $(document).ready(function() {
        ...
        if (localStorage.admin_token) {           
            ...
            $("#ajouter").attr('href', root_admin + '/settings/certification-titles/add');

            getCertificationTitle(null);
            ...
            $('#deleteJob').click(function() {
                ...
                console.log(line);
                disable_certification(id_skill, line);
            });                                
            ...
            $('#activateCt').click(function() {
                ...
                activate_certification(id_ct, line);
            });
    ...
    function getCertificationTitle(param){
        //var table;
        $.fn.dataTable.ext.errMode = 'throw'; //throw error on console
           ...
           var table = $('#listJobs').DataTable( {
                ...
                "searching": true,
                ajax:{
                    url: "../js/settings/certificationtitles/server-side-processing.php", // Change this URL to where your json data comes from
                ...
                "columnDefs": [                            
                    {
                        ...
                        "render": function ( data, type, row ) { 
                            ...
                            html += "<li><a href='"+root_admin+"/settings/certification-titles/"+row[2]+"' class='userProfil'>Edit</a></li>";
    ...
    function formattage(i, id_certi_title) {
        try {            
            ...
            $('#listJobs > tbody > tr:nth-child(' + i + ') > td:nth-child(3) > #action_edit_delete > ul > li:nth-child(5) > a').attr('data-reject', id_certi_title);
            $('#listJobs > tbody > tr:nth-child(' + i + ') > td:nth-child(3) > #action_edit_delete > ul > li:nth-child(3) > a').attr('data-reject', id_certi_title);            
    ...
    function disable_certification(id, line) {
        try {
            var uri;

            uri = config.api_url + '/certification-titles/' + id + '/disapprove';
            console.log(uri);
            $.ajax({
                url: uri,
                method: "POST",
...
//                            $('#listJobs > tbody > tr:nth-child(' + line + ')').attr('hidden', 'hidden');
    ...
    function activate_certification(id, line) {
        try {
            var uri;

            uri = config.api_url + '/certification-titles/' + id + '/approve';
            console.log(uri);
            $.ajax({
                url: uri,
                method: "POST",
...
//							}
    ...
    /*
    function get_certification_titles(direction) {
        try {
            var filters, status;
    ...
    function formattage(count) {
        try {
            for (i = 1; i <= count; i++) {
                ...
                $('#listJobs > tbody > tr:nth-child(' + i + ') > td:nth-child(2) > #action_edit_delete > ul > li:nth-child(1) > a').attr('href', root_admin + '/settings/certification-titles/' + id_job);
