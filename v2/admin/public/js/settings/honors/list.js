...
(function($) {
    $(document).ready(function() {
        ...
        if (localStorage.admin_token) {
            ...
            $("#ajouter").attr('href', root_admin + '/settings/honors/add');
...
//            });
    ...
    */        
    function getHonors(param){
        ...
        $.fn.dataTable.ext.errMode = 'throw'; //throw error on console
           ...
           var table = $('#listJobs').DataTable( {
                ...
                "searching": true,
                ajax:{
                    url: "../js/settings/honors/server-side-processing.php", // Change this URL to where your json data comes from
                ...
                "columnDefs": [                            
                    {
                        ...
                        "render": function ( data, type, row ) { 
                           var html = "<div class='btn-group' style='display:block;' id='action_edit_delete'>";
                            ...
                            html += "<li><a href='"+root_admin+"/settings/honors/"+row[2]+"' class='userProfil'>Edit</a></li>";
    ...
    function formattage(i, id_honor) {
        try {            
            ...
            $('#listJobs > tbody > tr:nth-child(' + i + ') > td:nth-child(3) > #action_edit_delete > ul > li:nth-child(5) > a').attr('data-reject', id_honor);
            $('#listJobs > tbody > tr:nth-child(' + i + ') > td:nth-child(3) > #action_edit_delete > ul > li:nth-child(3) > a').attr('data-reject', id_honor);            
    ...
    function disable_honor(id, line) {
        try {
            var uri;

            uri = config.api_url + '/honors/' + id + '/disapprove';
            console.log(uri);
            $.ajax({
                url: uri,
                method: "POST",
...
//                            $('#listJobs > tbody > tr:nth-child(' + line + ')').attr('hidden', 'hidden');
    ...
    function activate_honor(id, line) {
        try {
            var uri;

            uri = config.api_url + '/honors/' + id + '/approve';
            console.log(uri);
            $.ajax({
                url: uri,
                method: "POST",
    ...
    function formattage(count) {
        try {
            for (i = 1; i <= count; i++) {
                ...
                $('#listJobs > tbody > tr:nth-child(' + i + ') > td:nth-child(2) > #action_edit_delete > ul > li:nth-child(1) > a').attr('href', root_admin + '/settings/honors/' + id_job);
