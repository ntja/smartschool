...
class ForgotPasswordController extends Controller{
    ...
    public function post(Request $request) {
        try {    
			...
			$user = $current_account->checkUser($email);
			//var_dump($user);die();
			//send notification to user by email
			$user->notify(new ResetPassword($user,$secret));
			
			/*
			 $send = Mail::send('mails.reset-password', ['msg' => "Welcome on SmartSchool. This is the content of email.",'secret' => $secret], function ($m) use ($email, $email) {
			...
			*/
			$result = array(
				'code'=> '200',
				'message'=> "Instructions to reset your password has been sent to {$user->email}",
				"success" => 1,
			);
            return $result;
        } catch (Exception $e) {
