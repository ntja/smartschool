...
(function($) {
    $(document).ready(function() {
        ...
        if (localStorage.admin_token) {
            ...
            $("#ajouter").attr('href', root_admin + '/settings/industry/add');
            ...
            $('#deleteJob').click(function() {
                ...
                console.log(line);
                disable_industry(id_skill, line);
            });                                
        ...
        function getIndustry(param){
            ...
            $.fn.dataTable.ext.errMode = 'throw'; //throw error on console
               ...
               var table = $('#listJobs').DataTable( {
                    ...
                    "searching": true,
                    ajax:{
                        url: "../js/settings/industry/server-side-processing.php", // Change this URL to where your json data comes from
                    ...
                    "columnDefs": [                            
                        {
                            ...
                            "render": function ( data, type, row ) { 
                                ...
                                html += "<li><a href='"+root_admin+"/settings/industry/"+row[2]+"' class='userProfil'>Edit</a></li>";
        ...
        function formattage(i, id_industry) {
            try {  
                /*   
                ...
                //$('#listJobs > tbody > tr:nth-child(' + i + ') > td:nth-child(2) > #action_edit_delete > ul > li:nth-child(1) > a').attr('href', root_admin + '/settings/industry/' + id_job);
                   */
                ...
                $('#listJobs > tbody > tr:nth-child(' + i + ') > td:nth-child(3) > #action_edit_delete > ul > li:nth-child(5) > a').attr('data-reject', id_industry);
                $('#listJobs > tbody > tr:nth-child(' + i + ') > td:nth-child(3) > #action_edit_delete > ul > li:nth-child(3) > a').attr('data-reject', id_industry);            
        ...
        function activate_industry(id, line) {
        try {
            var uri;

            uri = config.api_url + '/industry/' + id + '/approve';
            console.log(uri);
            $.ajax({
                url: uri,
                method: "POST",
...
//							}
        ...
        function disable_industry(id, line) {   
        try {
            var uri;

            uri = config.api_url + '/industry/' + id + '/disapprove';
            console.log(uri);
            $.ajax({
                url: uri,
                method: "POST",
...
//                            $('#listJobs > tbody > tr:nth-child(' + line + ')').attr('hidden', 'hidden');
    ...
    /*
    function get_industry(direction) {
        try {
            var filters, status;
