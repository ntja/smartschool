...
(function($) {
    $(document).ready(function() {
        ...
        if (localStorage.admin_token) {
            // set up add job type link
            $("#ajouter").attr('href', root_admin + '/settings/job-types/add');
            ...
            $('#deleteJob').click(function() {
                ...
                console.log(line);
                disable_job_type(id_skill, line);
            });                                
...
//    });
    ...
    function getJobType(param){
        ...
        $.fn.dataTable.ext.errMode = 'throw'; //throw error on console
           ...
           var table = $('#listJobs').DataTable( {
                ...
                "searching": true,
                ajax:{
                    url: "../js/settings/jobtypes/server-side-processing.php", // Change this URL to where your json data comes from
                ...
                "columnDefs": [                            
                    {
                        ...
                        "render": function ( data, type, row ) { 
                            ...
                            html += "<li><a href='"+root_admin+"/settings/job-types/"+row[2]+"' class='userProfil'>Edit</a></li>";
    ...
    function formattage(i, id_job_type) {
        try {         
            ...
            $('#listJobs > tbody > tr:nth-child(' + i + ') > td:nth-child(3) > #action_edit_delete > ul > li:nth-child(5) > a').attr('data-reject', id_job_type);
            $('#listJobs > tbody > tr:nth-child(' + i + ') > td:nth-child(3) > #action_edit_delete > ul > li:nth-child(3) > a').attr('data-reject', id_job_type);            
    ...
    function disable_job_type(id, line) {
        try {
            var uri;

            uri = config.api_url + '/job-types/' + id + '/disapprove';
            console.log(uri);
            $.ajax({
                url: uri,
                method: "POST",
...
//                            $('#listJobs > tbody > tr:nth-child(' + line + ')').attr('hidden', 'hidden');
    ...
    function activate_job_type(id, line) {
        try {
            var uri;

            uri = config.api_url + '/job-types/' + id + '/approve';
            console.log(uri);
            $.ajax({
                url: uri,
                method: "POST",
    ...
    /*
    function get_jobtypes(direction) {
        try {
            var filters, status;
    ...
    function formattage(count) {
        try {
            for (i = 1; i <= count; i++) {
                ...
                $('#listJobs > tbody > tr:nth-child(' + i + ') > td:nth-child(2) > #action_edit_delete > ul > li:nth-child(1) > a').attr('href', root_admin + '/settings/job-types/' + id_job);
