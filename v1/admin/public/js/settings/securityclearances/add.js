...
(function($){
	...
	function add_security_clearance(){
		try{
			var uri, name;
			name = $('#input_name').val();
			uri = config.api_url+'/security-clearance-levels';
			data = {"name":name};
			$.ajax({
                url: uri,
                async: true,
			...
			.done(function(data, textStatus, jqXHR){
				...
				if(data.code == 200){
					...
					$("#msg").text("This security clearance name already exists");
					$('#flash_info').removeAttr('hidden');
					$("#save_btn").parent().removeClass('loading');
				}
			})
			.fail(function(jqXHR, textStatus, errorThrown){
				...
				if(response.code == 4000 || response.code == 400){
					$('#flash_info').attr('hidden', "hidden");
					$('#flash_info').removeClass('alert-success').addClass('alert-danger');
					$("#msg").text("Failed to add new security clearance");
					$('#flash_info').removeAttr('hidden');
				}else{
					$('#flash_info').attr('hidden', "hidden");
					$('#flash_info').removeClass('alert-success').addClass('alert-danger');
					$("#msg").text("Error : Failed to add new license");
					$('#flash_info').removeAttr('hidden');
				}
			});
		}catch(error){
