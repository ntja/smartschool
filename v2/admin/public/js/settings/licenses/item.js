...
(function($){
        ...
        if(localStorage.admin_token){
			// if(localStorage.admin_role == "ADMINISTRATOR"){				
				...
				$("#cancel").click(function(){
					window.location.href = root_admin+'/settings/licenses';
				});
			// }else{
	...
	function get_license(id){
		try{
			var uri = config.api_url+'/licenses/'+id;
	...
	function edit_license(id){
		try{
			var uri, name;
			name = $('#input_name').val();
			uri = config.api_url+'/licenses/'+id;
			data = {"name":name};
			$.ajax({
                url: uri,
                async: true,
