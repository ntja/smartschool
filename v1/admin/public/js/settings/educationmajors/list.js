...
(function($) {
    $(document).ready(function() {
        ...
        if (localStorage.admin_token) {
            ...
            $("#ajouter").attr('href', root_admin + '/settings/education-majors/add');
            ...
            $('#deleteJob').click(function() {
                ...
                console.log(line);
                disable_edu_major(id_skill, line);
            });                                
    ...
    function getEduMajors(param){
        ...
        $.fn.dataTable.ext.errMode = 'throw'; //throw error on console
           ...
           var table = $('#listJobs').DataTable( {
                ...
                "searching": true,
                ajax:{
                    url: "../js/settings/educationmajors/server-side-processing.php", // Change this URL to where your json data comes from
                ...
                "columnDefs": [                            
                    {
                        ...
                        "render": function ( data, type, row ) { 
                            ...
                            html += "<li><a href='"+root_admin+"/settings/education-majors/"+row[2]+"' class='userProfil'>Edit</a></li>";
    ...
    function activate_edu_major(id, line) {
        try {
            var uri;

            uri = config.api_url + '/education-majors/' + id + '/approve';
            console.log(uri);
            $.ajax({
                url: uri,
                method: "POST",
...
//							}
    ...
    function disable_edu_major(id, line) {
        try {
            var uri;

            uri = config.api_url + '/education-majors/' + id + '/disapprove';
            console.log(uri);
            $.ajax({
                url: uri,
                method: "POST",
...
//                            $('#listJobs > tbody > tr:nth-child(' + line + ')').attr('hidden', 'hidden');
    ...
    /*
    function get_education_majors(direction) {
        try {
            var filters, status;
    ...
    function formattage(count) {
        try {
            for (i = 1; i <= count; i++) {
                ...
                $('#listJobs > tbody > tr:nth-child(' + i + ') > td:nth-child(2) > #action_edit_delete > ul > li:nth-child(1) > a').attr('href', root_admin + '/settings/education-majors/' + id_job);
