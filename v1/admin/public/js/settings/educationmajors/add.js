...
(function($){
        ...
        if(localStorage.admin_token){
			// if(localStorage.admin_role == "ADMINISTRATOR"){				
				...
				$("#cancel").click(function(){
					window.location.href = root_admin+'/settings/education-majors';
				});
			// }else{
	...
	function add_job(){
		try{
			var uri, name;
			name = $('#input_name').val();
			uri = config.api_url+'/education-majors';
			data = {"name":name};
			$.ajax({
                url: uri,
                async: true,
			...
			.done(function(data, textStatus, jqXHR){
				...
				if(data.code == 200){
					...
					$("#msg").text("This education major already exists");
					$('#flash_info').removeAttr('hidden');
					$("#save_btn").parent().removeClass('loading');
				}
			...
			.fail(function(jqXHR, textStatus, errorThrown){
				...
				if(response.code == 4000 || response.code == 400){
					$('#flash_info').attr('hidden', "hidden");
					$('#flash_info').removeClass('alert-success').addClass('alert-danger');
					$("#msg").text("Failed to add new education major");
					$('#flash_info').removeAttr('hidden');
				}else{
					$('#flash_info').attr('hidden', "hidden");
					$('#flash_info').removeClass('alert-success').addClass('alert-danger');
