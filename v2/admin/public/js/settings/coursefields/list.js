...
(function($) {
    $(document).ready(function() {
        ...
        if (localStorage.admin_token) {            
            ...
            $("#ajouter").attr('href', root_admin + '/settings/course-fields/add');
            ...
            $('#deleteJob').click(function() {
                ...
                console.log(line);
                disable_course_field(id_skill, line);
            });                                
            ...
            $('#activateCt').click(function() {
                ...
                activate_course_field(id_ct, line);
            });
    ...
    function getCourseField(param){
        ...
        $.fn.dataTable.ext.errMode = 'throw'; //throw error on console
           ...
           var table = $('#listJobs').DataTable( {
                ...
                "searching": true,
                ajax:{
                    url: "../js/settings/coursefields/server-side-processing.php", // Change this URL to where your json data comes from
                ...
                "columnDefs": [                            
                    {
                        ...
                        "render": function ( data, type, row ) { 
                            ...
                            html += "<li><a href='"+root_admin+"/settings/course-fields/"+row[2]+"' class='userProfil'>Edit</a></li>";
    ...
    function formattage(i, id_course_field) {
        try {            
            ...
            $('#listJobs > tbody > tr:nth-child(' + i + ') > td:nth-child(3) > #action_edit_delete > ul > li:nth-child(5) > a').attr('data-reject', id_course_field);
            $('#listJobs > tbody > tr:nth-child(' + i + ') > td:nth-child(3) > #action_edit_delete > ul > li:nth-child(3) > a').attr('data-reject', id_course_field);            
    ...
    function disable_course_field(id, line) {
        try {
            var uri;

            uri = config.api_url + '/course-fields/' + id + '/disapprove';
            console.log(uri);
            $.ajax({
                url: uri,
                method: "POST",
...
//                            $('#listJobs > tbody > tr:nth-child(' + line + ')').attr('hidden', 'hidden');
    ...
    function activate_course_field(id, line) {
        try {
            var uri;

            uri = config.api_url + '/course-fields/' + id + '/approve';
            console.log(uri);
            $.ajax({
                url: uri,
                method: "POST",
...
//							}
    ...
    /*
    function get_course_fields(direction) {
        try {
            var filters, status;
    ...
    function formattage(count) {
        try {
            for (i = 1; i <= count; i++) {
                ...
                $('#listJobs > tbody > tr:nth-child(' + i + ') > td:nth-child(2) > #action_edit_delete > ul > li:nth-child(1) > a').attr('href', root_admin + '/settings/course-fields/' + id_job);
