...
(function($) {
    $(document).ready(function() {
        ...
        if (localStorage.admin_token) {
            // add a skill link
            $("#ajouter").attr('href', root_admin + '/settings/skills/add');
            
            getSkills(null);
            
            $('#listSkills').delegate('#action_edit_delete > ul > li:nth-child(5) > a', 'click', function() {
                id_skill = $(this).attr('data-reject');
                //role_user = $(this).attr('data-role');
                line = $(this).attr('data-line');
            });
            $('#listSkills').delegate('a.activate_link', 'click', function() {
    ...
    function activate_compname(id, line) {
        try {            
            var uri;

            uri = config.api_url + '/skills/' + id + '/approve';
            console.log(uri);
            $.ajax({
                url: uri,
                method: "POST",
            ...
            .done(function(data, textStatus, jqXHR) {
                ...
                $('.close').trigger('click');
                $('#listSkills > tbody > tr:nth-child(' + line + ')').find('.unactive').css('display', 'none');
                $('#listSkills > tbody > tr:nth-child(' + line + ')').find('.activ').css('display', 'block');
                //getSkills(null);
                ...
                $.ajax({
                    ...
                    crossDomain: true
                        ...
                        .done(function(data, textStatus, jqXHR) {
                            ...
                            $("#listSkills").DataTable().destroy();
                            // Utilisation de jQueryTemplate
                            $("#listSkills>tbody").loadTemplate($("#Ads"), data.items);

                            $('#listSkills').dataTable({
                                "deferRender": true,
                                "columnDefs": [
    ...
    */
    function getSkills(param){
        ...
        $.fn.dataTable.ext.errMode = 'throw'; //throw error on console
           ...
           var table = $('#listSkills').DataTable( {
                "processing": true,
                "serverSide": true,
                'info': true,
                "pagingType": "simple",
                ...
                "searching": true,
                ajax:{
                    url: "../js/settings/skills/server-side-processing.php", // Change this URL to where your json data comes from
                ...
                "columnDefs": [                            
                    {
                        ...
                        "render": function ( data, type, row ) { 
                            ...
                            html += "<li><a href='"+root_admin+"/settings/skills/"+row[2]+"' class='userProfil'>Edit</a></li>";
    ...
    */
    function formattage(i, id_skill) {
        try {            
            var is_active = $('#listSkills > tbody > tr:nth-child(' + i + ')').find('.activate_status').data('content');
            console.log(is_active);
            if (is_active != 0) {
                $('#listSkills > tbody > tr:nth-child(' + i + ')').find('.unactive').css('display', 'none');
                $('#listSkills > tbody > tr:nth-child(' + i + ')').find('.activ').css('display', 'block');

            } else {
                $('#listSkills > tbody > tr:nth-child(' + i + ')').find('.unactive').css('display', 'block');
                $('#listSkills > tbody > tr:nth-child(' + i + ')').find('.activ').css('display', 'none');
            }
            // Placement des marqueurs sur les bouton reject
            $('#listSkills > tbody > tr:nth-child(' + i + ') > td:nth-child(3) > #action_edit_delete > ul > li:nth-child(5) > a').attr('data-reject', id_skill);
            $('#listSkills > tbody > tr:nth-child(' + i + ') > td:nth-child(3) > #action_edit_delete > ul > li:nth-child(3) > a').attr('data-reject', id_skill);
            //$('#listSkills > tbody > tr:nth-child(' + i + ') > td:nth-child(3) > #action_edit_delete > ul > li:nth-child(4) > a').attr('data-reject', id_skill);
            // $('#listUsers > tbody > tr:nth-child('+i+') > td:nth-child(4) > #action_edit_delete > ul > li:nth-child(3) > a').attr('data-role', role_value);
            //$('#listSkills > tbody > tr:nth-child(' + i + ') > td:nth-child(3) > #action_edit_delete > ul > li:nth-child(4) > a').attr('data-line', i);
            $('#listSkills > tbody > tr:nth-child(' + i + ') > td:nth-child(3) > #action_edit_delete > ul > li:nth-child(5) > a').attr('data-line', i);
            $('#listSkills > tbody > tr:nth-child(' + i + ') > td:nth-child(3) > #action_edit_delete > ul > li:nth-child(3) > a').attr('data-line', i);
        } catch (error) {
            console.log('error');
        }
    }
    ...
    */
    function delete_skill(id, line) {
        try {
            var uri;

            uri = config.api_url + '/skills/' + id + '/disapprove';
            console.log(uri);
            $.ajax({
                url: uri,
                method: "POST",
            ...
            .done(function(data, textStatus, jqXHR) {
                console.log(data);
                if (data.code == 200) {
                    $('#deleteSkill').parent().addClass('success');
                    $('#deleteSkill').parent().removeClass('loading');
                    $('#deleteSkill').parent().removeClass('fail');

                    $('.close').trigger('click');
                    $('#listSkills > tbody > tr:nth-child(' + line + ')').find('.unactive').css('display', 'block');
                    $('#listSkills > tbody > tr:nth-child(' + line + ')').find('.activ').css('display', 'none');
                    //getSkills(null);
                    //$('span#status_pending').parent().removeClass('success');
                    //$('span#status_approved').parent().removeClass('success');
                    //$('#listJobs > tbody > tr:nth-child(' + line + ')>td:nth-child(2)>span').removeClass('label-success').addClass('label-default');
                    //$('#listJobs > tbody > tr:nth-child(' + line + ')>td:nth-child(2)>span').text("PENDING");
                }
            })
            .fail(function(jqXHR, textStatus, errorThrown) {
                console.log(jqXHR);
                ...
                if (jqXHR.status == 400) {
                    $('.close').trigger('click');
                    $('<a href="#result" data-toggle="modal">Info</a>').trigger('click');
                }
            });
        } catch (error) {
            console.log(error);
