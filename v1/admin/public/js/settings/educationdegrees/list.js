...
(function($) {
    $(document).ready(function() {
        ...
        if (localStorage.admin_token) {          
            ...
            $("#ajouter").attr('href', root_admin + '/settings/education-degrees/add');
            ...
            $('#deleteJob').click(function() {
                ...
                console.log(line);
                disable_edudegree(id_skill, line);
            });                                
    ...
    function getEduDegree(param){
        ...
        $.fn.dataTable.ext.errMode = 'throw'; //throw error on console
           ...
           var table = $('#listJobs').DataTable( {
                ...
                "searching": true,
                ajax:{
                    url: "../js/settings/educationdegrees/server-side-processing.php", // Change this URL to where your json data comes from
                ...
                "columnDefs": [                            
                    {
                        ...
                        "render": function ( data, type, row ) { 
                            ...
                            html += "<li><a href='"+root_admin+"/settings/education-degrees/"+row[2]+"' class='userProfil'>Edit</a></li>";
    ...
    function disable_edudegree(id, line) {
        try {
            var uri;

            uri = config.api_url + '/education-degrees/' + id + '/disapprove';
            console.log(uri);
            $.ajax({
                url: uri,
                method: "POST",
...
//                            $('#listJobs > tbody > tr:nth-child(' + line + ')').attr('hidden', 'hidden');
    ...
    function activate_edudegree(id, line) {
        try {
            var uri;

            uri = config.api_url + '/education-degrees/' + id + '/approve';
            console.log(uri);
            $.ajax({
                url: uri,
                method: "POST",
...
//							}
    ...
    /*
    function get_education_degrees(direction) {
        try {
            var filters, status;
    ...
    function formattage(count) {
        try {
            for (i = 1; i <= count; i++) {
                ...
                $('#listJobs > tbody > tr:nth-child(' + i + ') > td:nth-child(2) > #action_edit_delete > ul > li:nth-child(1) > a').attr('href', root_admin + '/settings/education-degrees/' + id_job);
