    ...
    public function post(Request $request) {
        try {                 			
			...
			if (is_null($key)) {
                ...
                $result = array("code" => 4000, "description" => "'key' parameter is required");
			...
			$confirm_password = array_key_exists("confirm_password", $data) ? $data["confirm_password"] : null;			
            try{
            ...
            $date_expiration = date("Y-m-d H:m:i",strtotime($date)+(7*24*60*60));
            ...
            $result = array("code" => 4000, "description" => "An internal error occured. Please try again later");
            echo json_encode($result, JSON_UNESCAPED_SLASHES);
            die(); 
        }
    }          
