...
(function($){
    $(document).ready(function(){
        ...
        if(localStorage.admin_token){
			// if(localStorage.admin_role == "ADMINISTRATOR"){				
				...
				secuID = url.segment(-1);
				
				/* Get secu info */
				get_security_clearance(secuID);
				...
				$("#cancel").click(function(){
					window.location.href = root_admin+'/settings/security-clearances';
				});
			// }else{
	...
	function validate_form() {
        ...
        try {
            ...
            if(validation == false){
                ...
                edit_security_clearance(secuID);
            }
        } catch (error) {
            console.log(error);
        }
	...
	function get_security_clearance(id){
		try{
			var uri = config.api_url+'/security-clearance-levels/'+id;
	...
	function edit_security_clearance(id){
		try{
			var uri, name;
			name = $('#input_name').val();
			uri = config.api_url+'/security-clearance-levels/'+id;
			data = {"name":name};
			$.ajax({
                url: uri,
                async: true,
			...
			.done(function(data, textStatus, jqXHR){
				...
				if(data.code == 200){
					...
					window.location.href = root_admin+'/settings/security-clearance-levels';
				}
			...
			.fail(function(jqXHR, textStatus, errorThrown){
				...
				if(jqXHR.responseJSON.code == 400){
					...
					$("#msg").text("Error : An error occured. Please try again later.");
					$('#flash_info').removeAttr('hidden');
				}else{
					$('#flash_info').attr('hidden', "hidden");
					$('#flash_info').removeClass('alert-success').addClass('alert-danger');
					$("#msg").text("Error : An internal error occured. Please try again later");
					$('#flash_info').removeAttr('hidden');
				}
			});
		}catch(error){
