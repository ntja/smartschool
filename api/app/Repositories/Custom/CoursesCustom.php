                ...
                if (array_key_exists('photo', $param)) {
                    if (!is_null($param['photo'])) {
                        if (!is_string($param['photo'])) {
                            $result = array("code" => 4000, "photo" => "photo is a string ");
                            echo json_encode($result, JSON_UNESCAPED_SLASHES);
                            return false;
                        }
                        if (empty(trim($param['photo']))) {
                            $result = array("code" => 4000, "photo" => "photo must not be empty ");
                            echo json_encode($result, JSON_UNESCAPED_SLASHES);
                            return false;
                        }
                    }                                    
    ...
    public function getList($params,$account){
        try{            
            ...
            if ($validate) {                         
                ...
                LogRepository::printLog('info', "All courses has been retrieve by user #".$account->getPropertyValue('id').". Request inputs: {" . var_export($params,true) . "}.");    
                return $rows;
            }            
        }catch(Exception $ex){
            LogRepository::printLog('error', $ex->getMessage());
