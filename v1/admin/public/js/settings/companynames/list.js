...
(function($) {
    $(document).ready(function() {
        ...
        if (localStorage.admin_token) {           
            ...
            $("#ajouter").attr('href', root_admin + '/settings/company-names/add');
            ...
            $('#deleteJob').click(function() {
                ...
                console.log(line);
                disable_company_name(id_skill, line);
            });                                
            ...
            $('#activateCt').click(function() {
                ...
                activate_company_name(id_ct, line);
            });
    ...
    function getCompanyName(param){
        ...
        $.fn.dataTable.ext.errMode = 'throw'; //throw error on console
           ...
           var table = $('#listJobs').DataTable( {
                ...
                "searching": true,
                ajax:{
                    url: "../js/settings/companynames/server-side-processing.php", // Change this URL to where your json data comes from
                ...
                "columnDefs": [                            
                    {
                        ...
                        "render": function ( data, type, row ) { 
                            ...
                            html += "<li><a href='"+root_admin+"/settings/company-names/"+row[2]+"' class='userProfil'>Edit</a></li>";
    ...
    function formattage(i, id_company_name) {
        try {            
            ...
            $('#listJobs > tbody > tr:nth-child(' + i + ') > td:nth-child(3) > #action_edit_delete > ul > li:nth-child(5) > a').attr('data-reject', id_company_name);
            $('#listJobs > tbody > tr:nth-child(' + i + ') > td:nth-child(3) > #action_edit_delete > ul > li:nth-child(3) > a').attr('data-reject', id_company_name);            
    ...
    function disable_company_name(id, line) {
        try {
            var uri;

            uri = config.api_url + '/company-names/' + id + '/disapprove';
            console.log(uri);
            $.ajax({
                url: uri,
                method: "POST",
...
//                            $('#listJobs > tbody > tr:nth-child(' + line + ')').attr('hidden', 'hidden');
    ...
    function activate_company_name(id, line) {
        try {
            var uri;

            uri = config.api_url + '/company-names/' + id + '/approve';
            console.log(uri);
            $.ajax({
                url: uri,
                method: "POST",
...
//							}
    ...
    /*
    function get_company_names(direction) {
        try {
            var filters, status;
    ...
    function formattage(count) {
        try {
            for (i = 1; i <= count; i++) {
                ...
                $('#listJobs > tbody > tr:nth-child(' + i + ') > td:nth-child(2) > #action_edit_delete > ul > li:nth-child(1) > a').attr('href', root_admin + '/settings/company-names/' + id_job);
